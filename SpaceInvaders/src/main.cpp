...
int main()
{
    ...
    while(!IsKeyPressed(KEY_ENTER)){
        ...
        const char* title = "Name presents : Space Invaders";
        ...
        DrawText(title, (windowWidth - titleWidth) / 2 +offset, windowHeight / 4, 30, WHITE);

        // Display description and instructions
        int descriptionWidth = MeasureText(description, 20);
        DrawText(description, (windowWidth - descriptionWidth) / 2 +offset, windowHeight / 4 + 50, 20, LIGHTGRAY);

        int controlsWidth = MeasureText(controls, 20);
        DrawText(controls, (windowWidth - controlsWidth) / 2 +offset, windowHeight / 4 + 100, 20, LIGHTGRAY);

        int fireWidth = MeasureText(fire, 20);
        DrawText(fire, (windowWidth - fireWidth) / 2 +offset, windowHeight / 4 + 130, 20, LIGHTGRAY);

        int livesWidth = MeasureText(lives, 20);
        DrawText(lives, (windowWidth - livesWidth) / 2 +offset, windowHeight / 4 + 160, 20, LIGHTGRAY);

        int startGameWidth = MeasureText(startGame, 20);
        DrawText(startGame, (windowWidth - startGameWidth) / 2 +offset, windowHeight / 4 + 200, 20, YELLOW);
